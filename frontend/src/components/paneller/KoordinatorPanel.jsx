    ...
    queryFn: async () => {
      ...
      return {
        kullanici: {
          ...
          beklemede: kullanicilar.data.filter(k => k.rol === "beklemede").length,
          koordinator: kullanicilar.data.filter(k => k.rol === "koordinator").length,
          arac_sahibi: kullanicilar.data.filter(k => k.rol === "arac_sahibi").length,
          talep_eden: kullanicilar.data.filter(k => k.rol === "talep_eden").length,
        },
        talep: {
          toplam: talepler.data.length,
          bekleyen: talepler.data.filter(t => t.durum === "beklemede").length,
          gorevlendirildi: talepler.data.filter(t => t.durum === "gorevlendirildi").length,
          iptal: talepler.data.filter(t => t.durum === "iptal edildi").length,
          tamamlanan: talepler.data.filter(t => t.durum === "tamamlandı").length,
        },
        gorev: {
          toplam: gorevler.data.length,
          bekleyen: gorevler.data.filter(g => g.gorevDurumu === "beklemede").length,
          basladi: gorevler.data.filter(g => g.gorevDurumu === "başladı").length,
          tamamlanan: gorevler.data.filter(g => g.gorevDurumu === "tamamlandı").length,
          iptal: gorevler.data.filter(g => g.gorevDurumu === "iptal edildi").length,

        },
     arac: {
    toplam: araclar.data.araclar.length,
    musait: araclar.data.araclar.filter(a => a.musaitlikDurumu === true).length,
    gorevde: araclar.data.araclar.filter(a => a.musaitlikDurumu === false).length,
    aktif: araclar.data.araclar.filter(a => a.aracDurumu === "aktif").length,
    pasif: araclar.data.araclar.filter(a => a.aracDurumu === "pasif").length,
  }
      };
    },
  });
